...
static AlarmType ParseAlarmType(const std::string &s)
{
	...
	if (s == "daily")
		return AlarmType::Daily;
	if (s == "weekly")
		return AlarmType::Weekly;
	if (s == "monthly")
		return AlarmType::Monthly;
	if (s == "interval")
		return AlarmType::Interval;
	return AlarmType::OneShot;
}
...
void AlarmManager::OnTimerFired()
{
	...
	for (auto &a : alarms_)
	{
		...
		if (a.enabled && a.next_trigger && a.next_trigger <= now)
		{
			...
			Application::GetInstance().RequestTts("%Alarm:到" + a.label + "的时间了,再大声提醒我一次,并询问我的完成情况");
...
{
	std::lock_guard<std::mutex> lk(mutex_);
	auto it = std::remove_if(alarms_.begin(), alarms_.end(), [id](const AlarmItem &a)
							 { return a.id == id; });
	if (it == alarms_.end())
		return false;
...
bool AlarmManager::EnableAlarm(int id, bool enable)
{
	...
	for (auto &a : alarms_)
	{
		if (a.id == id)
		{
			a.enabled = enable;
			if (enable)
				RecalculateNextTrigger(a, time(nullptr));
			else
				a.next_trigger = 0;
			SaveToSettings();
			ScheduleTimer();
...
void AlarmManager::AddMcpTools()
{
	...
	mcp.AddTool("self.alarm.add", "Add an alarm.", PropertyList({Property("type", kPropertyTypeString), Property("hour", kPropertyTypeInteger, 0, 23), Property("minute", kPropertyTypeInteger, 0, 59), Property("second", kPropertyTypeInteger, 0, 59), Property("day", kPropertyTypeInteger, 1, 31), Property("month", kPropertyTypeInteger, 1, 12), Property("year", kPropertyTypeInteger, 2024, 2100), Property("weekdays", kPropertyTypeInteger, 0, 0x7F), // bit mask
	...
	mcp.AddTool("self.alarm.list", "List all alarms.List all reminders", PropertyList(), [this](const PropertyList &) -> ReturnValue
				{ return ListAlarmsJson(); });

	mcp.AddTool("self.alarm.remove", "Remove an alarm by id.", PropertyList({Property("id", kPropertyTypeInteger, 0, 10000)}), [this](const PropertyList &props) -> ReturnValue
				{
		int id = props["id"].value<int>();
		return RemoveAlarm(id); });

	mcp.AddTool("self.alarm.enable", "Enable or disable an alarm.", PropertyList({Property("id", kPropertyTypeInteger, 0, 10000), Property("enable", kPropertyTypeBoolean)}), [this](const PropertyList &props) -> ReturnValue
				{
		int id = props["id"].value<int>();
